...
class GlDriver extends Driver {
	...
	**/
	...
	function getChannels(t:Texture) {
		return switch (t.internalFmt) {
			...
			case hxd.PixelFormat.DXT_FORMAT.RGBA_DXT1, hxd.PixelFormat.DXT_FORMAT.RGBA_DXT3, hxd.PixelFormat.DXT_FORMAT.RGBA_DXT5,
				hxd.PixelFormat.ASTC_FORMAT.RGBA_4x4, 0x805B, 0x8E8C: GL.RGBA;
	...
	function getBindType(t:h3d.mat.Texture) {
		...
		if (t.flags.has(Cube))
			return #if (hlsdl > version("1.15.0")) isArray ? GL.TEXTURE_CUBE_MAP_ARRAY : #end
		GL.TEXTURE_CUBE_MAP;
	...
	override function uploadTexturePixels(t:h3d.mat.Texture, pixels:hxd.Pixels, mipLevel:Int, side:Int) {
		...
		if (t.format.match(S3TC(_))) {
			if (t.flags.has(IsArray) || t.flags.has(Is3D))
				...
				#else
				throw "TextureArray support requires hlsdl 1.12+";
				#end
			else
		...
		var buffer:ArrayBufferView = switch (t.format) {
			...
			case RGBA16F, R16F, RG16F, RGB16F, RGBA16U, R16U, RG16U, RGB16U: new Uint16Array(@:privateAccess pixels.bytes.b.buffer, pixels.offset,
					dataLen >> 1);
	...
	override function uploadBufferData(b:h3d.Buffer, startVertex:Int, vertexCount:Int, buf:hxd.FloatBuffer, bufPos:Int) {
		...
		gl.bufferSubData(GL.ARRAY_BUFFER, startVertex * stride, streamData(data, bufPos * 4, vertexCount * stride), bufPos * 4 * STREAM_POS,
			vertexCount * stride);
		#else
	...
	override function setRenderTarget(tex:h3d.mat.Texture, layer = 0, mipLevel = 0, depthBinding:h3d.Engine.DepthBinding = ReadWrite) {
		...
		if (tex.depthBuffer != null && depthBinding != NotBound) {
			...
			if (tex.depthBuffer.hasStencil() && tex.depthBuffer.format == Depth24Stencil8) {
				...
				gl.framebufferTexture2D(GL.FRAMEBUFFER, GL.STENCIL_ATTACHMENT, GL.TEXTURE_2D,
					tex.depthBuffer.hasStencil() ? @:privateAccess tex.depthBuffer.t.t : null, 0);
			}
		} else {
			gl.framebufferTexture2D(GL.FRAMEBUFFER, GL.DEPTH_STENCIL_ATTACHMENT, GL.TEXTURE_2D, null, 0);
	...
	function captureSubRenderBuffer(pixels:hxd.Pixels, x:Int, y:Int) {
		...
		switch (curTarget.format) {
			...
			case RGBA16F, R16F, RG16F, RGB16F, RGBA16U, R16U, RG16U, RGB16U:
