...
private extern class GL2 extends js.html.webgl.GL {
	...
	function texSubImage3D(target:Int, level:Int, xoffset:Int, yoffset:Int, zoffset:Int, width:Int, height:Int, depth:Int, format:Int, type:Int,
		image:Dynamic):Void;
	function compressedTexSubImage3D(target:Int, level:Int, xoffset:Int, yoffset:Int, zoffset:Int, width:Int, height:Int, depth:Int, format:Int,
		image:Dynamic):Void;
	...
	**/
	...
	function selectMaterialBits(bits:Int) {
		...
		if (diff & (Pass.blendSrc_mask | Pass.blendDst_mask | Pass.blendAlphaSrc_mask | Pass.blendAlphaDst_mask) != 0) {
			...
			if (csrc == asrc && cdst == adst) {
				if (csrc == 0 && cdst == 1)
					...
					if (curMatBits < 0 || (Pass.getBlendSrc(curMatBits) == 0 && Pass.getBlendDst(curMatBits) == 1))
						gl.enable(GL.BLEND);
		...
		if (diff & Pass.depthTest_mask != 0) {
			...
			if (cmp == 0)
				...
				if (curMatBits < 0 || Pass.getDepthTest(curMatBits) == 0)
	...
	function selectStencilBits(opBits:Int, maskBits:Int) {
		...
		if (diffOp & (Stencil.frontSTfail_mask | Stencil.frontDPfail_mask | Stencil.frontPass_mask) != 0) {
			gl.stencilOpSeparate(FACES[Type.enumIndex(Front)], STENCIL_OP[Stencil.getFrontSTfail(opBits)], STENCIL_OP[Stencil.getFrontDPfail(opBits)],
				STENCIL_OP[Stencil.getFrontPass(opBits)]);
		}

		if (diffOp & (Stencil.backSTfail_mask | Stencil.backDPfail_mask | Stencil.backPass_mask) != 0) {
			gl.stencilOpSeparate(FACES[Type.enumIndex(Back)], STENCIL_OP[Stencil.getBackSTfail(opBits)], STENCIL_OP[Stencil.getBackDPfail(opBits)],
		...
		if ((diffOp & Stencil.frontTest_mask) | (diffMask & (Stencil.reference_mask | Stencil.readMask_mask)) != 0) {
			gl.stencilFuncSeparate(FACES[Type.enumIndex(Front)], COMPARE[Stencil.getFrontTest(opBits)], Stencil.getReference(maskBits),
				Stencil.getReadMask(maskBits));
		}

		if ((diffOp & Stencil.backTest_mask) | (diffMask & (Stencil.reference_mask | Stencil.readMask_mask)) != 0) {
			gl.stencilFuncSeparate(FACES[Type.enumIndex(Back)], COMPARE[Stencil.getBackTest(opBits)], Stencil.getReference(maskBits),
				Stencil.getReadMask(maskBits));
		}
	...
	override function clear(?color:h3d.Vector, ?depth:Float, ?stencil:Int) {
		...
		if (depth != null) {
			gl.depthMask(true);
			if (curMatBits >= 0)
				curMatBits |= Pass.depthWrite_mask;
			gl.clearDepth(depth);
		...
		if (bits != 0)
			gl.clear(bits);
		if (curTarget != null)
			curTarget.flags.set(WasCleared);
	}
	...
	function getChannels(t:Texture) {
		return switch (t.internalFmt) {
			...
			case hxd.PixelFormat.DXT_FORMAT.RGBA_DXT1, hxd.PixelFormat.DXT_FORMAT.RGBA_DXT3, hxd.PixelFormat.DXT_FORMAT.RGBA_DXT5,
				hxd.PixelFormat.ASTC_FORMAT.RGBA_4x4, hxd.PixelFormat.PVRTC_FORMAT.RGBA_4BPPV1: GL.RGBA;
	...
	override function allocTexture(t:h3d.mat.Texture):Texture {
		...
		inline function checkError() {
			...
			if (err != 0)
				throw "Failed to alloc texture " + t.format + "(error " + err + ")";
			return false;
		}
	...
	function expandStream(needed:Int) {
		...
		streamLen = newLen;
		if (streamBytes != null)
			streamKeep.push({f: frame, b: streamBytes});
	...
	function resetStream() {
		...
		while (streamKeep.length > 0 && streamKeep[0].f < frame - 1)
			streamKeep.shift();
		#end
	}
	...
	override function uploadTexturePixels(t:h3d.mat.Texture, pixels:hxd.Pixels, mipLevel:Int, side:Int) {
		...
		if (t.format.match(S3TC(_))) {
			if (t.flags.has(IsArray))
				...
				#else
				throw "TextureArray support requires hlsdl 1.11+";
				#end
			else
	...
	override function uploadVertexBuffer(v:VertexBuffer, startVertex:Int, vertexCount:Int, buf:hxd.FloatBuffer, bufPos:Int) {
		...
		gl.bufferSubData(GL.ARRAY_BUFFER, startVertex * stride * 4, streamData(data, bufPos * 4, vertexCount * stride * 4), bufPos * 4 * STREAM_POS,
	...
	override function uploadVertexBytes(v:VertexBuffer, startVertex:Int, vertexCount:Int, buf:haxe.io.Bytes, bufPos:Int) {
		...
		gl.bindBuffer(GL.ARRAY_BUFFER, v.b);
		#if hl
		gl.bufferSubData(GL.ARRAY_BUFFER, startVertex * stride * 4, streamData(buf.getData(), bufPos * 4, vertexCount * stride * 4), bufPos * 4 * STREAM_POS,
			vertexCount * stride * 4);
		#else
	...
	override function uploadIndexBuffer(i:IndexBuffer, startIndice:Int, indiceCount:Int, buf:hxd.IndexBuffer, bufPos:Int) {
		...
		#if hl
		var data = #if hl hl.Bytes.getArray(buf.getNative()) #else buf.getNative() #end;
		gl.bufferSubData(GL.ELEMENT_ARRAY_BUFFER, startIndice << bits, streamData(data, bufPos << bits, indiceCount << bits), (bufPos << bits) * STREAM_POS,
			indiceCount << bits);
		#else
	...
	override function uploadIndexBytes(i:IndexBuffer, startIndice:Int, indiceCount:Int, buf:haxe.io.Bytes, bufPos:Int) {
		...
		gl.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, i.b);
		#if hl
		gl.bufferSubData(GL.ELEMENT_ARRAY_BUFFER, startIndice << bits, streamData(buf.getData(), bufPos << bits, indiceCount << bits),
			(bufPos << bits) * STREAM_POS, indiceCount << bits);
		#else
	...
	override function setRenderTarget(tex:h3d.mat.Texture, layer = 0, mipLevel = 0) {
		...
		if (mipLevel > 0 && glES == 1)
			throw "Cannot render to mipLevel in WebGL1, use upload() instead";
		...
		if (tex.depthBuffer != null) {
			...
			if (tex.depthBuffer.hasStencil() && tex.depthBuffer.format == Depth24Stencil8) {
				...
				gl.framebufferRenderbuffer(GL.FRAMEBUFFER, GL.STENCIL_ATTACHMENT, GL.RENDERBUFFER,
					tex.depthBuffer.hasStencil() ? @:privateAccess tex.depthBuffer.b.r : null);
			}
		} else {
			gl.framebufferRenderbuffer(GL.FRAMEBUFFER, GL.DEPTH_STENCIL_ATTACHMENT, GL.RENDERBUFFER, null);
	...
	function checkFeature(f:Feature) {
		return switch (f) {
			...
			case FloatTextures if (glES >= 3): gl.getExtension('EXT_color_buffer_float') != null && gl.getExtension("OES_texture_float_linear") != null; // allow render to 16f/32f textures (not standard in webgl 2)
			...
			case MultipleRenderTargets: mrtExt != null || (mrtExt = gl.getExtension('WEBGL_draw_buffers')) != null;
	...
	function captureSubRenderBuffer(pixels:hxd.Pixels, x:Int, y:Int) {
		...
		switch (curTarget.format) {
			case RGBA32F, R32F, RG32F, RGB32F:
				buffer = new Float32Array(buffer.buffer);
			case RGBA16F, R16F, RG16F, RGB16F:
		...
		if (error != 0)
			throw "Failed to capture pixels (error " + error + ")";
	...
	static var TWRAP = [GL.CLAMP_TO_EDGE, GL.REPEAT,];

	static var FACES = [0, GL.FRONT, GL.BACK, GL.FRONT_AND_BACK,];

	static var BLEND = [
		GL.ONE, GL.ZERO, GL.SRC_ALPHA, GL.SRC_COLOR, GL.DST_ALPHA, GL.DST_COLOR, GL.ONE_MINUS_SRC_ALPHA, GL.ONE_MINUS_SRC_COLOR, GL.ONE_MINUS_DST_ALPHA,
		GL.ONE_MINUS_DST_COLOR, GL.CONSTANT_COLOR, GL.CONSTANT_ALPHA, GL.ONE_MINUS_CONSTANT_COLOR, GL.ONE_MINUS_CONSTANT_ALPHA, GL.SRC_ALPHA_SATURATE,
	];
